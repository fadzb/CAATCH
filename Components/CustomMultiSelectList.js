// code from 'react-native-multiple-select-list' component. Had to add customisations in order to deal with multi line labels
...
export default class CustomMultiSelectList extends Component {
  ...
  render() {
    ...
    const list = this.state.searchText
      ? this.filterObjectByValue(options, (option) => option.toLowerCase().includes(this.state.searchText))
      : options;
    ...
    return (
      ...
      >
        ...
        <ScrollView style={[{ padding: 5, height: this.props.scrollViewHeight }, this.props.scrollViewStyle]}>
