...
class VicChart extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            compareGraphData: [{x: 0, y: 0}, {x: 0, y: 0}],
            dateRange: [],
            graphReady: false,
            modalVisible: false,
        ...
        readDatabase('*', DbTableNames.diary, res => {
            ...
            scaleList.forEach((d, i) => {
                ...
                if(d.diaryName === USED_SKILLS || d.diaryName === STEPS) {
                    ...
                    compareList[d.diaryName] = scaleList.filter(t => t.diaryType === d.diaryType).reduce((obj, r, i) => {
    ...
    getDiaryData = () => {
        ...
        readDatabaseArg(columns,
            ...
            " where DATE(diaryDate) between Date('" + timeFrames.monthDate + "') and Date('" + today + "') and " + (this.props.settings.dbt ? "diaryType = 'Feeling' or diaryType = 'General'" : "diaryType = 'General'"));
    };
    ...
    transformFunction = (selectedDiaryItem, updateState) => {
        ...
        if(this.state.selectedTimeFrame === periods.week || this.state.selectedTimeFrame === periods.month) {
            ...
            this.setState({[updateState]: trackGraph.map(tr => ({...tr, x: Moment(tr.x).format("DD/MM")}))}, this.setState({graphReady: true}))
            ...
            if (this.state.checkedItemTime !== this.state.selectedTimeFrame) {
                this.setState({selectedTimeFrame: this.state.checkedItemTime}, () => {
                    if (this.state.checkedItemTime === periods.week) {
                        this.transformForGraph(this.state.selectedDiaryItem, periods.week, false, () => this.transformFunction(this.state.compareDiaryItem, 'compareGraphData'))
                    } else if (this.state.checkedItemTime === periods.month) {
                        this.transformForGraph(this.state.selectedDiaryItem, periods.month, false, () => this.transformFunction(this.state.compareDiaryItem, 'compareGraphData'))
                    } else if (this.state.checkedItemTime === periods.sixMonth) {
                        this.transformForGraph(this.state.selectedDiaryItem, periods.sixMonth, false, () => this.transformFunction(this.state.compareDiaryItem, 'compareGraphData'))
                    } else {
                        this.transformForGraph(this.state.selectedDiaryItem, periods.year, false, () => this.transformFunction(this.state.compareDiaryItem, 'compareGraphData'))
                    }
                });
            }

            if(this.state.checkedItemDiary !== this.state.selectedDiaryItem) {
                this.setState({selectedDiaryItem: this.state.checkedItemDiary, compareDiaryItem: 'None', checkedItemCompare: 'None'});
    ...
    getYCategorey = () => {
        const scale = this.state.ratingsInfo[this.state.selectedDiaryItem].scale === 'Undetermined' ? 5 : this.state.ratingsInfo[this.state.selectedDiaryItem].scale;
            ...
            <View style={TabStyles.stackContainer}>
                ...
                <View style={{backgroundColor: '#fff'}} collapsable={false} ref={ref => (this.chartView = ref)}>
                    <VictoryChart
                        ...
                        categories={{
                            [(this.state.ratingsInfo[this.state.selectedDiaryItem].scale !== 'Undetermined' || this.state.graphData.reduce((acc, g) => acc + g.y, 0) === 0) && 'y']: this.getYCategorey(),
                    ...
                    >
                        ...
                        <VictoryGroup data={this.state.graphData}
                                      domain={{[(this.state.ratingsInfo[this.state.selectedDiaryItem].scale !== 'Undetermined') && 'y']: this.getYDomain(this.state.selectedDiaryItem)}}
                                      >
                            <VictoryLine
                                style={{
