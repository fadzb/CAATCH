...
class Helper extends React.Component {
    static navigationOptions = ({ navigation }) => {
        return {
            ...
            headerRight: (
                <TouchableOpacity
                    onPress={() => navigation.push('newHelper')}
    ...
    getCompleteList = () => {
        readDatabaseArg("h.*, c.contactId, c.firstName, c.surname", DbTableNames.helper, this.updateHelpers, () => console.log("DB read success"), ' as h inner join ' + DbTableNames.contact + ' as c' +
    ...
    deleteHelper = (id) => {
        updateDatabaseArgument(DbTableNames.helper,
            ...
            ["dateDeleted"],
            "where helperId = " + id, () => console.log("deleting helper..."),
    ...
    renderItem = ({item}) => (
        <View style={helperStyle.listContainer}>
            <SafetyPlanSectionRow
                ...
                onPress={() => this.summaryNav(item.helperId, item.contactId, item.firstName + `${item.surname !== null ? ' ' + item.surname : ''}`, item.dateEntered, item.responsibility)}
                deleteFunction={() => this.showAlert(item.helperId)}
                editFunction={() => this.editHelper(item.helperId, item.firstName + `${item.surname !== null ? ' ' + item.surname : ''}`, item.responsibility, item.contactId)}
                icon={Icons.helpers}
            />
        </View>
    );
