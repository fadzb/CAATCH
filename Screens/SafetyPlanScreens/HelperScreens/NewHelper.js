...
export default class NewHelper extends React.Component {
    ...
    onPress = () => {
        if(this.state.resp.length === 0 && this.state.contact === 'Contact') {
            ...
            updateDatabase(DbTableNames.helper, [this.state.resp, this.props.navigation.getParam('checkedContact').contactId], ['responsibility', 'contactId'], this.updateHelperList({firstName: this.state.contact}), this.postSaveFunctions);
