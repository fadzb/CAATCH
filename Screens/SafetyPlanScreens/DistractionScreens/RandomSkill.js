...
export default class RandomSkill extends React.Component {
    ...
    updateSkills = (skills) => {
        this.setState({ skills: skills.filter(item => item.category === SafetyPlanConstants.randomSkill).map(skill => skill.name) }, () => {
    ...
    populateText = () => {
        ...
        if(maxIndex !== 0) {
            ...
            this.setState({text: this.state.unseenSkills[randIndex]}, () => this.setState(prevState => ({unseenSkills: prevState.unseenSkills.filter((sk, i) => i !== randIndex)})))
        } else {
            this.setState({text: this.state.unseenSkills[0]}, () => this.setState({unseenSkills: [...this.state.skills]}))
        }
    };
