...
export default class EnvironmentSafe extends React.Component {
  ...
  renderItem = ({ item }) => (
    <View style={environmentStyle.listContainer}>
      <EnvironmentRow description={item.environmentName} deleteFunction={() => this.showAlert(item.environmentId)} />
    </View>
  );
  ...
  handleSave = () => {
    if (!this.state.text) {
      ...
      updateDatabase(DbTableNames.environment, [this.state.text], ['environmentName'], undefined, (res) =>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 20 }}>
            {!this.state.inputHidden && (
              <TextInput
                style={{
                  height: 40,
                  borderColor: this.state.invalidText ? '#a94442' : 'gray',
                  borderWidth: 1,
                  paddingLeft: 5,
                  flex: 1,
                  marginRight: 15,
            ...
            <View
              style={{
                ...
                alignItems: this.state.inputHidden ? 'center' : 'flex-end',
              ...
              {!this.state.inputHidden && (
                <View style={{ flexDirection: 'row', marginRight: 5 }}>
                  ...
                  <Icon
                    ...
                    size={40}
                    onPress={() =>
                      this.setState((prevState) => ({ inputHidden: !prevState.inputHidden, invalidText: false }))
                    }
                  />
                </View>
              )}
