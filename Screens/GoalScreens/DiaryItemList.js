...
class DiaryItemList extends React.Component {
  ...
  componentDidMount() {
    readDatabaseArg(
      ...
      'where scale is not NULL' + (!this.props.settings.dbt ? ' and diaryType = "General"' : '')
    );
  }
      ...
      <View style={diaryItemStyle.viewContainer}>
        <View style={{ flex: 1, marginBottom: 50 }}>
          <CustomMultiSelectList
            ...
            selected={
              this.props.navigation.getParam('selectedDiaryItems') !== undefined
        ...
        <TouchableHighlight
          ...
          onPress={
            this.props.navigation.getParam('edit')
              ? () => {
                  const selectedObject = this.state.diaryList.find(
                    (di) => di.diaryId === Number(this.state.checkedItems[0])
                  );
