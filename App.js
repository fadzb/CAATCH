        ...
        // firstly checking if directory exists
            .then(fileObj => {
                if (fileObj.exists) {
                    ...
                    FileSystem.makeDirectoryAsync(file)
                        .catch(err => console.error(err));
                }
            })
            .then(() => FileSystem.getInfoAsync(file + 'caatch.db')
                // then check if file exists
                    .then(fileObj => {
                        if (fileObj.exists) {
                            ...
                            //download file here
                            FileSystem.downloadAsync(Asset.fromModule(require('./Database/caatch.db')).uri, file + "/caatch.db").
