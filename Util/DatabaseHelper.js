...
export const readDatabase = (column, table, resultFunc, func) => {
    db.transaction(tx => {
        tx.executeSql(`select ${column} from ${table}`, [], resultFunc !== undefined &&((_, resultSet) => (resultFunc(resultSet.rows._array))), (_, err) => console.log(err))
    }, err => console.log(err), func !== undefined &&(func))
...
export const updateDatabaseArgument = (tableName, writeData, columns, argument, func, resultFunc) => {
    ...
    db.transaction(tx => {
        tx.executeSql(`update ${tableName} set ${columnValues}` + argument, writeData, resultFunc !== undefined && ((_, res) => resultFunc(res)), err => console.log(err))
    }, err => console.log(err), func !== undefined &&(func))
};
...
export const readDatabaseArg = (column, table, resultFunc, func, argument) => {
    db.transaction(tx => {
        tx.executeSql(`select ${column} from ${table} ` + argument, [], resultFunc !== undefined &&((_, resultSet) => (resultFunc(resultSet.rows._array))), (_, err) => console.log(err))
    }, err => console.log(err), func !== undefined &&(func))
};
...
export const deleteDatabaseRow = (table, argument, resultFunc) => {
    db.transaction(tx => {
        tx.executeSql(`delete from ${table} ` + argument, [], resultFunc !== undefined &&((_, resultSet) => (resultFunc(resultSet.rows._array))))
    })
};